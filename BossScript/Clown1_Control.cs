...
public class Clown1_Control : MonoBehaviour {
    ...
    void Update()
    {
        ...
        // 몬스터의 좌, 우 이동과 공격
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("ClownBoy1_run"))
        {
            if (w_left == true)
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("ClownBoy1_attack1"))
        {
            ...
            if (!attack_on)  // 공격모션일경우 물방울 생성
            {
                attack_on = true;
                Debug.Log("순간이동!");
                if (playerCtrl.transform.localScale.x < 0)
                {
                    gameObject.transform.position = playerCtrl.transform.position + new Vector3(+2, 5, 0);
                    Walk_left();
                }
                else
                {
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("ClownBoy1_attack2"))
        {
            anim.SetBool("CountOver", false);
            damage_count = 0;
            ...
            if (!attack_on)  // 공격모션일경우 물방울 생성
            {
                ...
                Instantiate(mine, playerCtrl.transform.position + new Vector3(-5, 3, 0), Quaternion.identity);
                Instantiate(mine, playerCtrl.transform.position + new Vector3(-2.5f, 3, 0), Quaternion.identity);
                Instantiate(mine, playerCtrl.transform.position + new Vector3(2.5f, 3, 0), Quaternion.identity);
                Instantiate(mine, playerCtrl.transform.position + new Vector3(5, 3, 0), Quaternion.identity);
            }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (col.gameObject.tag == "Attack"
            && damage_count <= 10
            && bossHP > 0
            && (anim.GetCurrentAnimatorStateInfo(0).IsName("ClownBoy1_run"))) // 이 오브젝트의 콜라이더안에 들어온 Trigger의 tag가 Attack라면 피격 애니메이션을 보여주고 HP를 10깎는다.
        {
            Debug.Log("공격바듬");
