...
public class Helmet4_Control : MonoBehaviour
{
    Manager mng;
    ...
    void Update()
    {
        ...
        // 몬스터의 좌, 우 이동과 공격
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy4_run"))
        {
            if (w_left == true)
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy4_attack_1"))
        {
            anim.SetBool("CountOver", false);
            damage_count = 0;

            if(!attack_on)  // 공격모션일경우 물방울 생성
            {
                attack_on = true;
                
                Instantiate(aquaBall, transform.FindChild("AquaSpawn1").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn2").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn3").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn4").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn5").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn6").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn7").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn8").position, Quaternion.identity);
                Instantiate(aquaBall, transform.FindChild("AquaSpawn9").position, Quaternion.identity);
            }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (col.gameObject.tag == "Attack"
            && damage_count <= 10
            && bossHP > 0
            && (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy4_run"))) // 이 오브젝트의 콜라이더안에 들어온 Trigger의 tag가 Attack라면 피격 애니메이션을 보여주고 HP를 10깎는다.
        {
            Debug.Log("공격바듬");
