...
public class Helmet_Control : MonoBehaviour
{
    Manager mng;
    public int Speed;
    public float bossHP;
    ...
    void Update()
    {
        ...
        // 몬스터의 좌, 우 이동
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy_walk"))
        {
            attacked = false;
        ...
        if (detect)
        {
            if (Mathf.Abs(GetDistanePlayerX()) < 11.0f )
            {
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.tag == "Attack" 
            && damage_count <= 10
            && bossHP > 0
            &&( anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy_walk") 
            || anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy_idle"))
        ...
        if (col.gameObject.tag == "Player")
        {
            ...
            else
            {
                if (col.transform.position.x < transform.position.x)
                {
                    col.GetComponent<Rigidbody2D>().AddForce(new Vector2(-200.0f, 2.0f));
                }
                else
                    col.GetComponent<Rigidbody2D>().AddForce(new Vector2(200.0f, 2.0f));
            }
    ...
    public void JumpAttack()
    {
        ...
        if(w_left)
        {
            GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x - 5, 10);
        }
        else
            GetComponent<Rigidbody2D>().velocity = new Vector2(GetComponent<Rigidbody2D>().velocity.x + 5, 10);
    }
