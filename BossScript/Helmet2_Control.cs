...
public class Helmet2_Control : MonoBehaviour
{
    Manager mng;
    ...
    void Update()
    {
        ...
        // 몬스터의 좌, 우 이동과 공격
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy2_run"))
        {
            if (w_left == true)
        ...
        if (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy2_attack"))
        {
            damage_count = 0;
            anim.SetBool("CountOver", false);
            attack_timer = 0;
            if (w_left == true)
                transform.Translate(Speed * -0.7f * Time.deltaTime, 0.0f, 0.0f);
            else
                transform.Translate(Speed * 0.7f * Time.deltaTime, 0.0f, 0.0f);
        }
        ...
        if (detect)
        {
            if (Mathf.Abs(GetDistanePlayerX()) < 11.0f && ready)
            {
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if (col.gameObject.tag == "Attack"
            && damage_count <= 10
            && bossHP > 0
            && (anim.GetCurrentAnimatorStateInfo(0).IsName("HelmetBoy2_run"))
        ...
        if (col.gameObject.tag == "Player")
        {
            ...
            else
            {
                if (col.transform.position.x < transform.position.x)
                {
                    col.GetComponent<Rigidbody2D>().AddForce(new Vector2(-200.0f, 0.0f));
                }
                else
                    col.GetComponent<Rigidbody2D>().AddForce(new Vector2(200.0f, 0.0f));
            }
